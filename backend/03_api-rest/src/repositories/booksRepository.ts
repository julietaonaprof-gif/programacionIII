...
/**
 * Capa de acceso a datos para la entidad Book.
 * Toda interacción con SQLite vive aquí; ningún método conoce HTTP.
 */
export class BooksRepository {
  ...
  findAll(filters: FindAllFilters, page: number, limit: number): Promise<FindAllResult> {
    return new Promise((resolve, reject) => {
      ...
      // Primero contamos el total para armar la paginación
      this.db.get(
  ...
  /**
   * Actualiza sólo los campos presentes en `fields`.
   * Retorna `true` si se modificó alguna fila, `false` si el libro no existe.
   */
  patch(isbn: string, fields: BookPatch): Promise<boolean> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.db.run(
        `UPDATE books SET ${setClauses} WHERE isbn = ?`,
        [...values, isbn],
        function (err) {
          if (err) return reject(err);
